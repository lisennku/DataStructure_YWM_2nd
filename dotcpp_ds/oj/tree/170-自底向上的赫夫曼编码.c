...
//
// https://www.dotcpp.com/oj/problem1700.html
...
void hfm_tree_code_create(HuffmanTree hf, HuffmanCode * code, int size) {
    *code = (char **) calloc(size + 1, sizeof(char *));
    ...
    for(int i = 1; i <= size; i++) {
        int start = size - 1;
        int parent_idx = hf[i].parent;
        int child_idx = i;

        while(parent_idx != 0) {
            if(hf[parent_idx].left == child_idx)
                tmp[--start] = '0';
            else
                tmp[--start] = '1';
            child_idx = parent_idx;
            parent_idx = hf[parent_idx].parent;

        }
        (*code)[i] = calloc(size - start, sizeof(char));
