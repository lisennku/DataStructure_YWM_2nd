...
void create_algraph(ALGraph * graph, int numCourses, int** prerequisites, int prerequisitesSize) {
    ...
    for(int i = 0; i < prerequisitesSize; i++) {
        int edge_h = prerequisites[i][0];
        int edge_t = prerequisites[i][1];
...
bool topo_sort(ALGraph graph) {
    // 拓扑序列数组
    int topo[graph.vertex_nums];
    int topo_idx = 0;
    ...
    return topo_idx == graph.vertex_nums ? true : false;
...
int main() {
    ...
    int numCourses = 2;
    int **prerequisites = malloc(sizeof(int *) * 2);
    ...
    create_algraph(&g, numCourses, prerequisites, 2);
    bool flag = topo_sort(g);
