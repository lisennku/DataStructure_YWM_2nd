...
void ShowFavouriteBoosk(Book list, int n, char arr[][MAX_TITL]);
...
int main() {
	...
	int favourite_nums;
	scanf("%d", &favourite_nums);
	char names[favourite_nums][MAX_TITL];
	for(int i = 0; i < favourite_nums; i++)
		scanf("%s", &names[i]);
	ShowFavouriteBoosk(list, favourite_nums, names);
