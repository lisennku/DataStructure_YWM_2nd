...
double CalculatePriceAvg(Book list);
void ChangeBookList(Book list);
...
double CalculatePriceAvg(Book list) {
	...
	return sum_total / GetListLength(list);
}

void ChangeBookList(Book list) {
	double avg = CalculatePriceAvg(list);
	Node * p = list->next;
	while(p != NULL) {
		if(p->price < avg)
			p->price *= 1.2;
